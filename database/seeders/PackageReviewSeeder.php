...
class PackageReviewSeeder extends Seeder
{
    ...
    {
        $half_packages = (int) DB::table('packages')->count() / 2;
        $user_ids = User::where('user_type', UserTypeEnum::USER->value)->take(25)->pluck('id')->all();
        DB::transaction(function () use ($half_packages, $user_ids) {
            Package::active()->take($half_packages)->each(function ($package) use ($user_ids) {
                $temp = [];
                ...
                $package->reviews()->createMany($temp);
            });
        });
    }
}
